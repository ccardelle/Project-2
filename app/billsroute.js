...
module.exports = function(app, passport) {
  ...
  app.get("/deleteBill/:id", isLoggedIn, (req, res, next) => {
    Bills.findByIdAndRemove(req.params.id)
      .then(data => {
        console.log(data);
        res.redirect("/profile");
      })
  ...
  app.post("/crowdCheck", isLoggedIn, (req, res, next) => {
    console.log(
      "THIS IS THE RESULT OF THE CROWD SEARCH ////////////////// " +
        req.body.name
    );
    Bills.find({ name: req.body.name })
      .then(bills => {
        console.log(
          "Crowds result search +++++++++++++++++++++++++++ " + bills
        );
